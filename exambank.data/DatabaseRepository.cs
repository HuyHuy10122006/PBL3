...
namespace exambank.data
{
    ...
    public class DatabaseRepository : IDatabaseRepository
    {
        ...
        public DatabaseRepository(ExamBankDbContext dbContext)
        {
            ...
            // Tự động tạo DB và bảng nếu chưa có (dành cho chế độ LocalDB)
            try
            {
                _dbContext.Database.EnsureCreated();

                // Đảm bảo có User và Category mặc định với Id=1 để không lỗi khoá ngoại khi AddQuestion
                if (!_dbContext.Users.Any(u => u.Id == 1))
                {
                    _dbContext.Database.ExecuteSqlRaw("SET IDENTITY_INSERT Users ON; INSERT INTO Users (Id, Username, Password, Email, Role, FullName, CreatedAt, IsActive) VALUES (1, 'admin', 'admin', 'admin@local', 'Admin', 'Admin', GETDATE(), 1); SET IDENTITY_INSERT Users OFF;");
                }
                if (!_dbContext.Categories.Any(c => c.Id == 1))
                {
                    _dbContext.Database.ExecuteSqlRaw("SET IDENTITY_INSERT Categories ON; INSERT INTO Categories (Id, Name, Description, CreatedAt, IsActive) VALUES (1, N'Default', N'Default Category', GETDATE(), 1); SET IDENTITY_INSERT Categories OFF;");
                }
            }
            catch (Exception ex)
            {
